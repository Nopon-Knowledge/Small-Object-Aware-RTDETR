...
JOB_NAME="${JOB_NAME:-rtdetr_eval_wheat_tsecaf_r18}"
PYTHON_BIN="${PYTHON_BIN:-$HOME/venvs/rtdetr_env/bin/python}"
TEST_SCRIPT="${TEST_SCRIPT:-tools/test_gwhd_a40.py}"
...
case "$SUBMIT_BIN" in
  jsub)
    jsub \
      ...
      -cwd "$PROJECT_DIR" \
      "$PYTHON_BIN" "$TEST_SCRIPT" --preset "$PRESET" "${EXTRA_ARGS[@]}"
    ;;
  sbatch)
    printf -v WRAP_CMD '%q ' "$PYTHON_BIN" "$TEST_SCRIPT" --preset "$PRESET" "${EXTRA_ARGS[@]}"
    sbatch \
      --partition="$QUEUE" \
      --gres="gpu:${GPU_COUNT}" \
      --job-name="$JOB_NAME" \
  ...
  "")
    ...
    printf 'cd %q && %q %q --preset %q' "$PROJECT_DIR" "$PYTHON_BIN" "$TEST_SCRIPT" "$PRESET" >&2

...
JOB_NAME="${JOB_NAME:-rtdetr_vis_wheat_tsecaf_r18}"
PYTHON_BIN="${PYTHON_BIN:-$HOME/venvs/rtdetr_env/bin/python}"
VIS_SCRIPT="${VIS_SCRIPT:-tools/visualize_gwhd_predictions.py}"
PRESET="${PRESET:-wheat_tsecaf_r18_val_best}"
...
case "$SUBMIT_BIN" in
  jsub)
    jsub \
      ...
      -cwd "$PROJECT_DIR" \
      "$PYTHON_BIN" "$VIS_SCRIPT" --preset "$PRESET" "${EXTRA_ARGS[@]}"
    ;;
  sbatch)
    printf -v WRAP_CMD '%q ' "$PYTHON_BIN" "$VIS_SCRIPT" --preset "$PRESET" "${EXTRA_ARGS[@]}"
    sbatch \
      --partition="$QUEUE" \
      --gres="gpu:${GPU_COUNT}" \
      --job-name="$JOB_NAME" \
  ...
  "")
    ...
    printf 'cd %q && %q %q --preset %q' "$PROJECT_DIR" "$PYTHON_BIN" "$VIS_SCRIPT" "$PRESET" >&2
